...
\date{
  ~
  \\[2em]
  \textsf{\LARGE \textsc{EDP y Métodos Numéricos.} \\[1ex] \textit{Máster Interuniversitario en Matemáticas}}
  \\[7em]
...
\item \texttt{Atom} on GNU/Linux, MacOS or Windows. Like Emacs, Atom is a
powerful free/libre general-purpose text editor.
...
Here we review the configuration of the last three ones.
...
\subsection{Using \FF under Atom}
Atom is a trendy and stylish general purpose free (as freedom) web editor
which can be installed following the instructions in its
\href{https://atom.io}{web page}.

Thousands of packages are available which make Atom do amazing
things. They can be installed using the menu \textit{Atom $\rightarrow$
Preferences $\rightarrow$ Install}. Among them, the packages
\textit{language-freefem-official} and \textit{freefem-runner}
can be recommended for syntax highlighting and running FreeFem++ scripts,
using the menu \textit{Packages $\rightarrow$ FreeFem++ Runner $\rightarrow$
Launch compilation} or pressing the keys \textit{Alt + Control + F}.
...
mesh is defined by the subdivision of $[0,1]$ into 2 sub-intervals
...
plot(Th);
cout << "This first mesh works properly!" << endl;
\end{lstlisting}
...
mesh\footnote{Although in \FFcs, windows are opened in the right top
  panel}.  Although this graphic window contains no buttons or menus, it can be
controlled by the keyboard and by variables passed when
plotting. It can be closed using the \texttt{Esc} key. Press the '\texttt{?}'
key for help. For details see the \FF manual or the
