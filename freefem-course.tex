...
\lstinputlisting[caption={First example: Poisson problem homogeneous Dirichlet conditions}]{code/poisson-first-example.edp}
...
\lstinputlisting[caption={Poisson problem with mixed Dirichlet/Neumann boundary conditions}]{code/poisson-Dirich-Neumann.edp}
...
\begin{equation*}
  \begin{cases}
    ...
    \begin{aligned}
      \int_\Omega \nabla u_h \cdot \nabla v_h
      + \frac a b \int_{\Gamma_1} u v
      = \int_\Omega f\, v_h
      + \frac 1b \int_{\Gamma_1} g_1\, v_h
      \quad\text{for each } v_h\in U_{h},
    \end{aligned}
  \end{cases}
\end{equation*}
...
A finite element approximattion of the solution can be obtained by the following FreeFem++ script:

\lstinputlisting[caption={Poisson problem with mixed Dirichlet/Robin boundary conditions}]{code/poisson-Dirich-Robin.edp}
...
\lstinputlisting{code/heat-EI.edp}
