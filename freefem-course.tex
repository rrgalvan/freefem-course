...
% ecuaciÃ³n anterior, obtendremos el siguiente sistema en tiempo: En la
% etapa inicial ($m=0$), tomamos $u^0=u(t=0)=u_0$. Conocidos los valores
% $u^0,...,u^m$, que aproximan a los valores de $u(t_0),...,u(t_m)$, en
% la etapa $m+1$, calculamos $u^{m+1}$ como soluciÃ³n de: hallar $u\in
% H^1(\Omega)$ tal que $u=g_D$ en $\Gamma_D$, $u=g_N$ en $\Gamma_N$ y
% \begin{equation*}
%   \left\{
%     \begin{aligned}
%       &\int_\Omega\frac{u^{m+1}-u^m}{dt} v + \nu\int_\Omega \nabla u \cdot \nabla v
%         = \int_\Omega f \cdot v
%       \\
...
%   \right.
% \end{equation*}
% Como se comentÃ³ antes, el problema anterior puede reformulado para
% adaptarlo al marco de Lax-Milgram y, asÃ­, demostrar que existe una
% Ãºnica soluciÃ³n dÃ©bil. A continuaciÃ³n, si pasamos al segundo miembro
% los datos conocidos, podemos identificar las formas bilineal y lineal
% siguientes:
% \begin{equation*}
%     \begin{aligned}
%       a(u,v)&=\int_\Omega\frac{u^{m+1}}{dt} v + \nu\int_\Omega \nabla
%       u \cdot \nabla v,
%       \\
%       b(v)  &= \int_\Omega f \cdot v
%         +\int_\Omega \frac{u^{m}}{dt} v
%     \end{aligned}
% \end{equation*}
% y a partir de ahÃ­ escribir el programa que se muestra a continuaciÃ³n, que
% calcularÃ¡ la soluciÃ³n en cada etapa de tiempo (a travÃ©s de un bucle
% \verb|for|) y, en cada etapa, grabarÃ¡ en un fichero postscript la
% soluciÃ³n correspondiente.
